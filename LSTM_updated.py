...
player_data = df_sorted[df_sorted['Player Name'] == 'Schauffele, Xander']  # Example player
...
## PREDICT RORY MCILROYS PREVIOUS ROUND
...
# Prepare the dataset
X = player_data_one_hot[feature_columns]
y = player_data_one_hot[target_column]

full_length = sequence_length + 1

full_x = X[-full_length:].values
#full_y = y[-full_length:].values
full_y  = 71.75
...
x_scaled = scaler.fit_transform(test_x)

X_test_tensor = torch.tensor(x_scaled, dtype=torch.float32)
y_test_tensor = torch.tensor(test_y, dtype=torch.float32).view(-1, 1)
# Assuming test_x is your input sequence of shape (4, 60)
# Creating a tensor from the scaled data and adding a batch dimension
X_test_tensor = torch.tensor(x_scaled, dtype=torch.float32).unsqueeze(0)

# Setting the model to evaluation mode and making a prediction
model.eval()  # Evaluation mode
with torch.no_grad():
    prediction = model(X_test_tensor)
    predicted_score = prediction.item()  # Assuming your model outputs a single value per sequence
